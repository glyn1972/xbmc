...
bool CTagLoaderTagLib::ParseTag(ID3v2::Tag *id3v2, EmbeddedArt *art, MUSIC_INFO::CMusicInfoTag& tag)
{
  ...
  for (ID3v2::FrameListMap::ConstIterator it = frameListMap.begin(); it != frameListMap.end(); ++it)
  {
    ...
    else if (it->first == "TSOA")   tag.SetAlbumSort(it->second.front()->toString().to8Bit(true));
    ...
    else if (it->first == "TSOT")   tag.SetTitleSort(it->second.front()->toString().to8Bit(true));
...
bool CTagLoaderTagLib::ParseTag(MP4::Tag *mp4, EmbeddedArt *art, CMusicInfoTag& tag)
{
  ...
  for (auto it = itemMap.begin(); it != itemMap.end(); ++it)
  {
    if (it->first == "\251nam")
      ...
      tag.SetTitleSort(it->second.toStringList().front().to8Bit(true));
      ...
      tag.SetAlbumSort(it->second.toStringList().front().to8Bit(true));
